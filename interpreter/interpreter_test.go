...
import (
	...
	"github.com/bobicloudvision/davi/interpreter"
	"github.com/bobicloudvision/davi/parser"
)
...
func TestMain(m *testing.M) {
	flag.StringVar(&exePath, "exe", "", "path to Go davi interpreter binary")
	flag.StringVar(&interpPath, "interp", "", "path to davi.ll")
	flag.Parse()
	os.Exit(m.Run())
}
...
`, "", "1\n2\n3"},
		...
		// exit() builtin
		// Skip these for now as they exit the davi.ll version:
		// {`exit()`, "", "exit(0)"},
		// {`exit(42)`, "", "exit(42)"},
...
[["a", 42], ["a", 43], ["B", 42], ["z", 0]]`},
	...
	// Run tests against external davi interpreter
	if exePath != "" {
		for _, test := range tests {
